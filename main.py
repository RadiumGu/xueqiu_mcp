...
def process_data(data, process_config=None):
    ...
    """
    import copy
    data = copy.deepcopy(data)  # 避免修改原始数据

    if process_config is None:
        process_config = {'convert_timestamps': True}
...
@mcp.tool()
def quotec(stock_code: str="HK00700") -> dict:
    """获取某支股票的行情数据"""
    try:
...
@mcp.tool()
def quote_detail(stock_code: str="HK00700") -> dict:
    """获取某支股票的行情数据-详细"""
    try:
...
@mcp.tool()
def pankou(stock_code: str="HK00700") -> dict:
...
@mcp.tool()
def earningforecast(stock_code: str="HK00700") -> dict:
    """按年度获取业绩预告数据"""
    try:
...
@mcp.tool()
def report(stock_code: str="HK00700") -> dict:
    """获取机构评级数据"""
    try:
...
@mcp.tool()
def capital_flow(stock_code: str="HK00700") -> dict:
    """获取当日资金流如流出数据，每分钟数据"""
    try:
...
@mcp.tool()
def capital_history(stock_code: str="HK00700") -> dict:
    """获取历史资金流如流出数据，每日数据"""
    try:
...
@mcp.tool()
def capital_assort(stock_code: str="HK00700") -> dict:
    """获取资金成交分布数据"""
    try:
...
@mcp.tool()
def blocktrans(stock_code: str="HK00700") -> dict:
    """获取大宗交易数据"""
    try:
...
@mcp.tool()
def margin(stock_code: str="HK00700") -> dict:
    """获取融资融券数据"""
    try:
...
@mcp.tool()
def indicator(stock_code: str="HK00700", is_annals: int = 1, count: int = 5) -> dict:
    """按年度、季度获取业绩报表数据
...
@mcp.tool()
def income(stock_code: str="HK00700", is_annals: int = 1, count: int = 5) -> dict:
    """获取利润表数据
...
@mcp.tool()
def balance(stock_code: str="HK00700", is_annals: int = 1, count: int = 5) -> dict:
    """获取资产负债表数据
...
@mcp.tool()
def cash_flow(stock_code: str="HK00700", is_annals: int = 1, count: int = 5) -> dict:
    """获取现金流量表数据
...
@mcp.tool()
def business(stock_code: str="HK00700", count: int = 5) -> dict:
    """获取主营业务构成数据
...
@mcp.tool()
def top_holders(stock_code: str="HK00700", circula: int = 1) -> dict:
    """获取十大股东数据
...
@mcp.tool()
def main_indicator(stock_code: str="HK00700") -> dict:
    """获取F10主要指标数据"""
    try:
...
@mcp.tool()
def holders(stock_code: str="HK00700") -> dict:
    """获取F10股东人数数据"""
    try:
...
@mcp.tool()
def org_holding_change(stock_code: str="HK00700") -> dict:
    """获取F10机构持仓数据"""
    try:
...
@mcp.tool()
def bonus(stock_code: str="HK00700", page: int = 1, size: int = 10) -> dict:
    """获取F10分红融资数据
...
@mcp.tool()
def industry_compare(stock_code: str="HK00700") -> dict:
    """获取F10行业对比数据"""
    try:
...
@mcp.tool()
def nav_daily(cube_symbol: str="HK00700") -> dict:
    """获取组合净值数据
    ...
    try:
        ...
        return {"error": str(e), "cube_symbol": cube_symbol}


@mcp.tool()
def rebalancing_history(cube_symbol: str="HK00700") -> dict:
    """获取组合历史交易信息
...
def convertible_bond(page_size: int = 5, page_count: int = 1) -> dict:
    ...
    try:
        ...
        return {"error": str(e), "page_size": page_size}


@mcp.tool()
def index_basic_info(index_code: str="HK00700") -> dict:
    """获取指数基本信息
...
@mcp.tool()
def index_details_data(index_code: str="HK00700") -> dict:
    """获取指数详细信息
...
@mcp.tool()
def index_weight_top10(index_code: str="HK00700") -> dict:
    """获取指数权重股前十
...
@mcp.tool()
def index_perf(index_code: str="HK00700", days: int=30) -> dict:
    """获取指数近期收益数据

    Args:
        index_code: 指数代码
        days: 周期天数，支持 7、30、90
    """
    try:
        if days == 7:
            ...
            return {"error": f"days must be 7, 30, or 90, got {days}"}
        return process_data(result)
    except Exception as e:
        return {"error": str(e), "index_code": index_code, "days": days}
...
@mcp.tool()
def fund_info(fund_code: str="HK00700") -> dict:
    """获取基金基本信息
...
@mcp.tool()
def fund_growth(fund_code: str="HK00700") -> dict:
    """获取基金增长数据
...
@mcp.tool()
def fund_nav_history(fund_code: str="HK00700") -> dict:
    """获取基金历史净值数据
...
@mcp.tool()
def fund_achievement(fund_code: str="HK00700") -> dict:
    """获取基金业绩表现数据
...
@mcp.tool()
def fund_asset(fund_code: str="HK00700") -> dict:
    """获取基金资产配置数据
...
@mcp.tool()
def fund_manager(fund_code: str="HK00700") -> dict:
    """获取基金经理信息
...
@mcp.tool()
def fund_trade_date(fund_code: str="HK00700") -> dict:
    """获取基金交易日期信息
...
@mcp.tool()
def fund_derived(fund_code: str="HK00700") -> dict:
    """获取基金衍生数据
...
@mcp.tool()
def suggest_stock(keyword: str="腾讯") -> dict:
    """关键词搜索股票代码
